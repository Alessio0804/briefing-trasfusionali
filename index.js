...
import { summarizeRegion } from "./summarize.js";
...
async function main() {
  ...
  // Sintesi per regione: una chiamata semplice per ciascuna regione con novità.
  // Se una singola regione fallisce o torna vuota, si ripiega sull'elenco grezzo solo per quella.
  let posted = 0;
  for (const r of withItems) {
    let body;
    try {
      body = await summarizeRegion(r.name, r.items, label);
      if (!body || body.length < 20) throw new Error("risposta vuota");
    } catch (e) {
      console.warn(`Sintesi ${r.name} fallita, uso il fallback grezzo:`, e.message);
      body = fallbackBlock(r.items);
    }
